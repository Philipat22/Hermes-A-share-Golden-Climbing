...
MODEL_HORIZON = {mc['name']: mc['horizon'] for mc in MODEL_CFG}
WF_WINDOWS = [
...
# Forward returns (vectorized: ~10x faster than per-symbol loop)
print("[Computing forward returns...]")
for mc in MODEL_CFG:
    h = mc['horizon']; col = f'fwd_{h}d'
    pdf[col] = pdf.groupby('vt_symbol')['close'].transform(
    ...
    for wi, (tr_s, tr_e, te_s, te_e, wname) in enumerate(WF_WINDOWS):
        ...
        for di, d in enumerate(te_dates):
            ...
            for sym in list(positions.keys()):
                ...
                if dd <= STOP_LOSS:
                    ...
                    window_trades.append({
                        'window': wname, 'regime': pos.get('entry_regime', regime), 'model': model_name,
            ...
            if max_pos > 0:  # skip if regime says no trading
                ...
                cur_horizon = MODEL_HORIZON.get(model_name, 5)
                if open_slots > 0 and di < len(te_dates) - cur_horizon:
                    ...
                    if len(valid) > 0:
                        ...
                        if cand.sum() > 0:
                            ...
                            sell_date = te_dates[min(di + cur_horizon, len(te_dates) - 1)]
...
for key in ['5d_only', 'dual']:
    ...
    if len(df_t) > 0:
        df_t.to_csv(os.path.join(ROOT, 'quant_archive', '2026-05', f'backtest_v2_{key}_trades.csv'), index=False)
    if len(df_e) > 0:
        df_e.to_csv(os.path.join(ROOT, 'quant_archive', '2026-05', f'backtest_v2_{key}_equity.csv'), index=False)
