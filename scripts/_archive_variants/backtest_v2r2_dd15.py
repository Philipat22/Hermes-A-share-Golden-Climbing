"""
V2r2 + 15% Equity Curve Drawdown Protection

Modification over V2r2:
  - When equity drops >15% from peak → liquidate all, freeze 5 days
  - After 5 days → resume normal operation, reset peak tracker
  - Same underlying models and strategy as V2r2 (only structural risk overlay)
"""
...
# ── Drawdown protection ─────────────────────────────────────────
DD_THRESHOLD = 0.15      # 15% drawdown triggers protection
DD_FREEZE_DAYS = 5        # freeze days after trigger
...
def build_strat(mode='5d_only'):
    """Same as V2r2 — regime → (model_name, threshold, max_positions, capacity_pct)."""
    base = {
...
# ════════════════════════════════════════════════════════════════════
print("=" * 72)
print("V2R2 + 15% EQUITY DRAWDOWN PROTECTION")
print("=" * 72)
t0 = time.time()
...
def run_backtest(mode_label, strategy):
    """Same as V2r2 but with equity curve drawdown protection."""
    print(f"\n{'='*72}")
    ...
    for wi, (tr_s, tr_e, te_s, te_e, wname) in enumerate(WF_WINDOWS):
        ...
        peak_value = 1_000_000  # running peak equity
        dd_frozen_until = None  # date until which we stay frozen
        ...
        for di, d in enumerate(te_dates):
            ...
            # (Must run before stop-loss checks, using yesterday's total)
            # Tracked after mark-to-market below, so we use previous iteration's peak_value
            # For day 0: initialized to 1M
            in_protection = False
            if dd_frozen_until is not None and d < dd_frozen_until:
                in_protection = True
            elif dd_frozen_until is not None and d >= dd_frozen_until:
                # Freeze expired — reset
                dd_frozen_until = None
                peak_value = 1_000_000  # reset peak from current level
                in_protection = False

            # ── FORCED LIQUIDATION if drawdown triggered today ─────
            # (Sell everything before stop-loss checks)
            if in_protection:
                for sym in list(positions.keys()):
                    ...
                    window_trades.append({
                        ...
                        'symbol': sym, 'exit_reason': 'dd_protection',
            ...
            # ── SELECT new picks ──
            if not in_protection and max_pos > 0:
            ...
            # ── Daily mark-to-market & drawdown tracking ──
            pos_value = 0
            ...
            # Update peak and check drawdown (for NEXT day's check)
            if total > peak_value:
                peak_value = total
            dd_from_peak = (peak_value - total) / peak_value

            if dd_from_peak > DD_THRESHOLD and not in_protection:
                # Drawdown triggered — will enter protection tomorrow
                # (Trades today already executed; tomorrow's loop will force liquidate)
                dd_frozen_until = te_dates[min(di + DD_FREEZE_DAYS + 1, len(te_dates) - 1)]
                # Force liquidation NOW (end of day)
                for sym in list(positions.keys()):
                    pos = positions.pop(sym)
                    sell_price = get_price(sym, d, 'close')
                    if sell_price is None: continue
        ...
        if n > 0:
            ...
            # Protection stats
            dd_protect_trades = (trades_df['exit_reason'] == 'dd_protection').sum()
            days_protected = int(df_val['in_protection'].sum())
            pct_protected = df_val['in_protection'].mean() * 100
            ...
            print(f"    DD-Protect: {dd_protect_trades}x forced exits | {days_protected}d protected ({pct_protected:.1f}%)")
    ...
    if len(df_t) > 0:
        ...
        print(f"\n    Exit reasons:")
        for r in ['matured', 'stop_loss', 'dd_protection']:
...
# ════════════════════════════════════════════════════════════════════
results = {}
for mode_name, mode_key in [('A) 5d_10% + DD15', '5d_only'),
                             ('B) Dual Model + DD15', 'dual')]:
...
for key in ['5d_only', 'dual']:
    ...
    if len(df_t) > 0:
        df_t.to_csv(os.path.join(ROOT, 'quant_archive', '2026-05', f'backtest_v2r2_dd15_{key}_trades.csv'), index=False)
    if len(df_e) > 0:
        df_e.to_csv(os.path.join(ROOT, 'quant_archive', '2026-05', f'backtest_v2r2_dd15_{key}_equity.csv'), index=False)
