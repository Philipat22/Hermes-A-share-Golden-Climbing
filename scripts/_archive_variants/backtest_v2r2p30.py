...
for key in ['5d_only', 'dual']:
    ...
    if len(df_t) > 0:
        df_t.to_csv(os.path.join(ROOT, 'quant_archive', '2026-05', f'backtest_v2r2p30_{key}_trades.csv'), index=False)
    if len(df_e) > 0:
        df_e.to_csv(os.path.join(ROOT, 'quant_archive', '2026-05', f'backtest_v2r2p30_{key}_equity.csv'), index=False)
