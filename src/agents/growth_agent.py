...
def analyze_insider_conviction(trades: list) -> dict:
    """
    Analyze A-share margin balance trends as a proxy for market sentiment.
    
    In A-share context, this is NOT insider trading but margin (融资融券) data.
    We analyze day-over-day margin_balance (rzye) and short_balance (rqye) changes.
    """
    if len(trades) < 2:
        return {"score": 0.5, "net_flow_ratio": 0, "buys": 0, "sells": 0}
    
    sorted_trades = sorted(trades, key=lambda t: t.trade_date)
    
    # Compute net trend: margin balance changes day-over-day
    bullish_days = 0
    bearish_days = 0
    ...
    if total_days == 0:
        ...
        net_flow_ratio = (bullish_days - bearish_days) / total_days
